...
**Установив сервер, его необходимо запустить, и здесь появляются два варианта работы** **приложения**
...
<img src="./rezhimy-raboty.png" title="Установите службу соответствующей кнопкой, после чего запустите ее" crop="0,0,100,100" scale="100" width="699px" height="660px" float="center"/>

> [!TIP]
> 
> Иногда Windows может требовать учетные данные пользователя для управления службой -- в таком случае включите **Расширенный режим** и укажите логин/пароль, с которым входите в аккаунт Windows при запуске пк. Если у вас нет пароля для входа в систему, создайте его.

</tab>

<tab name="Консольное приложение">

**Режим консоли -- это формат работы, при котором вы вручную запускаете сервер как консоль. В таком случае сервер не будет запускаться автоматически при старте системы. Данный режим подходит для сбора ошибок, или если требуется единоразовый запуск приложения.**
