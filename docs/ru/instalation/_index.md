...
-  <icon code="server" color="#3b82f6"/> **Сервер под приложение и базу данных** -- наше облако или собственный пк в клубе, выделенный под сервер.
...
Подробнее -- в статье «Личный кабинет».
...
-  Установите базу данных -- для этого загрузите ее по [этой ссылке](https://download.microsoft.com/download/5/1/4/5145fe04-4d30-4b85-b0d1-39533663a2f1/SQL2022-SSEI-Expr.exe).

-  Откройте установщик базы данных и дождитесь инициализации.

-  В открывшемся меню выберите «Базовая», после чего «Установить». После завершения процесса установки закройте приложение. Теперь можно приступать к GizmoServer.
...
Подробный разбор мастера настройки со всеми параметрами и скриншотами -- в статье [«Мастер настройки Gizmo Server»](./gizmo/self-hosted/gizmo-server/windows/setup-wizard.md).
...
Как это сделать и как узнать локальный IP сервера -- в статье [«Доступ к Gizmo по сети»](./../tutorials/client-shell/network-access.md).
...
Подробнее о том, как вести сеть из нескольких клубов на одном сервере -- в статье [«Несколько клубов на одном сервере»](./../tutorials/operations/multiple-clubs-setup.md).
...
Подробнее про тарифы и расписания -- в статье [«Как настроить тарифы»](./../tutorials/sales/pricing/tariffs-guide.md).
...
Группа пользователей -- это категория клиентов со своими ценами, скидками и доступами. Например, вы можете создать «Зарегистрированные» -- обычные клиенты, «Гости» -- разовые аккаунты, «VIP» -- премиум со скидкой 10%, «Staff» -- сотрудники со скидкой 100% и без ограничений системы. Подробнее про группы пользователей -- в статье [«Группы пользователей»](./../tutorials/staff/user-groups.md).
...
Укажите цену пакета в деньгах и, если нужно, в баллах. Если оплата возможна и деньгами, и баллами на выбор -- переключите «И» на «ИЛИ». Баллы -- часть бонусной системы: вы начисляете клиентам баллы за покупки, а они тратят их на товары и пакеты по установленной вами цене. Подробнее о бонусной системе и баллах -- в статье [«Бонусные баллы для клиентов»](./../tutorials/loyalty/loyalty-program.md).

![](./instalation-12.png){width=618px height=899px}
...
Полный разбор всех настроек пакетов времени -- в статье [«Пакеты времени»](./../tutorials/sales/pricing/time-packages-guide.md).
...
-  В настройках подключения укажите **Имя хоста** (IP сервера) и **Порт** (по умолчанию 80). Как узнать локальный IP сервера -- в статье [«Доступ к Gizmo по сети»](./../tutorials/client-shell/network-access.md).
...
Настройка Companion на этом закончена, но касса и терминал ещё не подключены технически -- это отдельный процесс, и мы помогаем с ним удалённо. Чек-лист подготовки к обращению в поддержку -- в статье «Подключение кассы и терминала».
...
-  Перейдите в <cmd text="Настройки"/> -> <cmd text="Кассы"/> и выберите пк, с которого будете продавать. Если касс несколько и вы не уверены, какая из них какому пк соответствует -- смотрите статью «Подключение кассы и терминала».
...
Полный разбор всех полей карточки товара, а также наборы (комбо из нескольких товаров) -- в статье [«Товары и наборы»](./../tutorials/sales/shop/running-the-shop.md).
...
Затем привяжите склад к кассе: <cmd text="Настройки"/> -> <cmd text="Кассы"/> -> выберите кассу пк администратора и укажите нужный склад -- так все продажи с неё будут списываться именно с него. Если оставить оба склада, товары придётся вручную перемещать между ними -- это даёт полный контроль, но легко запутаться. Если это не нужно, проще оставить один склад.

Подробный разбор пополнений, списаний, переводов между складами и инвентаризации -- в статье [«Склад»](./../tutorials/sales/shop/warehouse.md).
...
Откройте <cmd text="Настройки"/> -> <cmd text="Права доступа"/>. Здесь уже есть готовые наборы прав -- откройте любой, чтобы увидеть, что в него входит. Можно использовать готовые наборы или создать свои. Чем отличаются готовые наборы друг от друга -- в статье [«Как выбрать готовый набор прав доступа»](./../tutorials/staff/permission-presets.md).
...
Теперь сотрудник сможет входить в Gizmo Manager и пользоваться тем, что вы ему разрешили. Учтите: этот аккаунт не даёт входа в клиентскую оболочку -- если хотите дать сотрудникам бесплатное или льготное время на пк, это отдельная настройка, разобранная в статье [«Льготы для персонала»](./../tutorials/staff/staff-perks.md).
...
Остальные параметры раздела можно менять на своё усмотрение, но лучше сначала разобраться, за что каждый отвечает -- разбор ключевых сценариев работы с оболочкой собран в теме [«Клиентский шелл»](./../tutorials/client-shell/_index.md).
...
Оболочка выглядит по-своему -- но пока в ней нечего запускать. Чтобы гости могли открывать игры и программы, добавьте их как приложения. Тема большая, разберём только самое нужное для старта -- полное руководство по приложениям и всем их настройкам в статье [«Как добавить игру в клиентскую оболочку»](./../tutorials/client-shell/add-app-to-shell.md).
...
2. Откроется папка с игрой -- скопируйте путь из адресной строки проводника, например `C:\Program Files (x86)\Steam`.

![](./instalation-15.png){width=1122px height=330px}

3. В поля **Путь к исполняемому файлу** и **Рабочий каталог** впишите этот путь, добавив в конце название запускаемого файла: `C:\Program Files (x86)\Steam\steam.exe`.
...
Это максимальный уровень блокировки -- он закрывает гостю почти всё за пределами оболочки. Через профили можно так же блокировать конкретные приложения, процессы и даже сайты в браузере. Полный разбор всех параметров -- в статье [«Профили безопасности»](./../tutorials/client-shell/security-profiles.md).
...
-  В поле **Имя хоста** укажите `gizmo.local` или локальный IP сервера. Зачем это нужно и как узнать локальный IP -- в статье [«Доступ к Gizmo по сети»](./../tutorials/client-shell/network-access.md).
...
-  Укажите путь хранения копий (по умолчанию `C:\Program Files\NETProjects\Gizmo Server\data\Backup`). Рекомендуем выбрать путь не на системном диске -- тогда копии сохранятся, даже если сломается сам системный диск. Это по-настоящему важная настройка -- не пропускайте её.

Про резервное копирование в облако -- в статье [«Резервное копирование в облако»](./../tutorials/operations/cloud-backup.md).
...
Гости часто забывают выходить из игр и лаунчеров, авторизованных на пк -- это создаёт риски и недоразумения между клубом и следующим гостем. Настройте автоматическую очистку сессий при выходе, чтобы это не было ручной работой -- подробнее в статье [«Очистка сессий игр и лаунчеров»](./../tutorials/client-shell/session-cleanup.md).
...
> [!CAUTION]
> 
> **Сталкиваетесь с проблемой при подключении?** Бесконечно висит этап загрузки, иконка сервера в левом нижнем углу экрана красная, устройство не появляется в Gizmo Manager -- или другие проблемы? Смотрите статью [«Проблемы подключения клиентских пк»](./../tutorials/client-shell/client-connection-troubleshooting.md) -- там разобраны частые причины, прежде чем обращаться в поддержку.
...
-  [«Продажи»](./../tutorials/sales/_index.md) -- тарифы, пакеты времени, кассы и терминалы, товары и склад.

-  [«Система лояльности»](./../tutorials/loyalty/_index.md) -- бонусные баллы, скидки и акции.

-  [«Бронирование»](./../tutorials/booking/_index.md) -- как закрепить игровое место за гостем заранее.

-  [«Клиентский шелл»](./../tutorials/client-shell/_index.md) -- приложения, сеть, безопасность и типовые проблемы клиентских пк.

-  [«Клиенты и персонал»](./../tutorials/staff/_index.md) -- группы пользователей, аккаунты, права доступа и льготы для сотрудников.

-  [«Управление клубом»](./../tutorials/operations/_index.md) -- карта, мониторинг, отчёты, очередь, удалённое управление, филиалы и резервные копии.