...
Это и есть рекомендация из статьи [«Установка»](./../../instalation/_index.md) -- выбирать именно **Перезагрузку**, а не более мягкие варианты вроде «Закрытие всех программ». Разница на первый взгляд небольшая, но именно она отделяет чистый пк от пк с чужими открытыми сессиями.
...
> [!NOTE]
> 
> Точный список поддерживаемых способов принудительного разлогина конкретных лаунчеров (например, отдельная настройка для Steam) появится в этой статье отдельно, после сверки с актуальным интерфейсом. Общая рекомендация до тех пор -- отключать сохранение пароля и автовход прямо в самом лаунчере при первоначальной настройке образа клиентского пк, до того как этот образ размножат на все станции клуба.
