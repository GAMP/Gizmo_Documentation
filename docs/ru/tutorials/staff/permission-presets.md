...
> [!NOTE]
> 
> Точный список всех готовых наборов, которые есть в системе "из коробки", и подробное сравнение того, чем каждый из них отличается от остальных (кроме уже подтверждённого «Кассира»), появится в этой статье отдельно -- после сверки с актуальным интерфейсом.
