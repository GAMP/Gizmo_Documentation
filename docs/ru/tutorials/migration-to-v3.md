...
-  Запустите сервер как консольное приложение или [как службу](https://gizmopowered.helpdeskeddy.com/ru/knowledge_base/article/24/category/6) (ПОМЕНЯТЬ ССЫЛКУ И РУКОВОДСТВО).

> [!WARNING]
> 
> При первом запуске может возникнуть ошибка -- чтобы исправить ее, просто закройте приложение и откройте снова

-  Теперь можно открыть новый **GizmoManager** по адресу `https://gizmo.local/apps/manager`. Эта ссылка будет актуальна для всего времени работы, поэтому если вам привычнее опыт использования десктопных приложений, установите менеджер как приложение (ДОБАВИТЬ ССЫЛКУ НА СТАТЬЮ ПРО УСТАНОВКУ МЕНЕДЖЕРА В КАЧЕСТВЕ АППА ИЗ БРАУЗЕРА).
...
**После успешной авторизации вы наконец попадете на карту управления клубом. Вздохните. Если глаза устали от яркого света -- включите темную тему, открыв профиль оператора в верхнем правом углу. Не спешите начинать работу, ведь есть еще несколько важных моментов, которые нужно проверить.**
...
1. Перейдите в раздел **Настройки -> Операторы и настройки прав доступа -> Права доступа** и нажмите **«Создать право доступа»**. Укажите название группы -- например, «Кассир», «Управляющий», «Руководитель» -- и выберите нужные права доступа. Полное описание всех прав доступно здесь (НАПИСАТЬ СТАТЬЮ ПО ВСЕМ ПРАВАМ).
...
<img src="./migration-to-v3-3.png" crop="0,0,100,100" objects="annotation,36.5794,40.8989,Проверить порт сервера можно в Настройках > Сеть,top-left" width="1919px" height="1079px" float="center"/>
...
<highlight color="purple">**Только для клубов с подключенным к gizmo ККТ или терминалом эквайринга:**</highlight>
...
Для работы **сканера** и денежного ящика перейдите в настройки **Gizmo Companion -> POS-устройства** и укажите COM-порты для устройств. В том же компаньоне, в разделе **настроек фискального принтера**, укажите данные для работы с ККМ-сервером.
...
<highlight color="red">**Эти настройки не переносятся автоматически при миграции с V2 на V3 -- их нужно указать заново.**</highlight>
...
> [!IMPORTANT]
> 
> Подробнее о всех возможностях настройки профиля оболочки мы рассказываем в этой статье. (СДЕЛАТЬ СТАТЬЮ ПРО ПРОФИЛИ)
