...
**Время создания резервной копии** -- время, в которое будет создана резервная копия. По умолчанию -- 0:00 (в полночь).

## Путь хранения резервной копии на сервере

**Путь хранения резервной копии на сервере** -- путь в файловой системе, по которому будут сохраняться резервные копии.
