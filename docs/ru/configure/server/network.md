...
**Имя хоста** -- доменное имя, по которому ваш сервер доступен в сети.
...
> [!WARNING]
> 
> Указание имени хоста обязательно для использования функций удалённого просмотра и удалённого управления.
...
Блок в нижней части страницы. Представляет собой таблицу из двух столбцов -- **Функция** и **Включить** -- со списком функций, которые включаются переключателем.

-  **Только зарегистрированные** -- подключение производится только для зарегистрированных хостов в системе.

   > [!WARNING]
   > 
   > Важное уточнение: если данная галочка включена -- сервер автоматически отбросит подключение нового компьютера с установленным клиентом. Чтобы зарегистрировать клиент, потребуется [добавить его вручную](./../../usage/gizmo-manager/stations/computers/computer-card.md) или отключить галочку на время.

-  **Восстановление имени хоста** -- при включении Gizmo назначает клиентскому ПК то имя, которое указано в поле **Имя хоста** [карточки компьютера](./../../usage/gizmo-manager/stations/computers/computer-card.md), и делает это при каждом подключении.